...
void ListGraph::AddEdge(int from, int to) {
    ...
    if (std::find(adjLists[from].begin(), adjLists[from].end(), to) != adjLists[from].end()) {
        return;
    }
    if (std::find(prevAdjLists[to].begin(), prevAdjLists[to].end(), from) != prevAdjLists[to].end()) {
        return;
    }
...
int ListGraph::VerticesCount() const { return adjLists.size(); }
